module Map3(Map, emptyM, assocM, lookupM, deleteM, keys) where

data Map k v = M [k] [v]
{- INV.REP.: 
    - en M ks vs, no hay claves repetidas en ks.
...
assocM2 k v (M ks vs) = armarM (asociarKV k v ks vs)

-- Costo: O(1)
armarM :: ([k],[v]) -> Map k v
armarM (ks,vs) = M ks vs

-- Costo: O(n), siendo n la cant de elementos de las listas.
asociarKV :: Eq k => k -> v -> [k] -> [v] -> ([k],[v])
asociarKV k v []      vs      = ([k],[v])
asociarKV k v (k':ks) (v':vs) =
	if k == k'
		then ((k':ks), (v:vs))
		else agregarPar k' v' (asociarKV k v ks vs)
...
-- Propósito: borra una asociación dada una clave.
-- Costo: O(n), siendo n la cantidad de pares del map.
-- esta implementación la hace parcial. debe existir el par con la clave dada en el map.
