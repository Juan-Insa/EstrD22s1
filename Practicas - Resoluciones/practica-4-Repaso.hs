...
manada2 = M (Cazador "pichicho" ["1","2","3","4"] 
...
agregarExploradoresA :: Nombre -> [Territorio] -> [(Territorio, [Nombre])] -> [(Territorio, [Nombre])]
agregarExploradoresA n []     ys = ys
agregarExploradoresA n (t:ts) ys = agregarExplorador n t (agregarExploradoresA n ts ys)
...
agregarExplorador n t []     = (t, n : []) : []
agregarExplorador n t (x:xs) = agregarExploradorA n t x : (agregarExplorador n t xs)
...
agregarExploradorA n t p = 
	if t == fst p
		then agregarNombre n p
		else p
...
-- dado un nombre de cazador y un lobo, indica el nombre de todos los cazadores que tienen
-- como subordinado al cazador dado (directa o indirectamente). 
-- Precondición: hay un cazador con dicho nombre y es único.
superioresDelCazadorL :: Nombre -> Lobo -> [Nombre]
superioresDelCazadorL n1 (Cria       n2)             = []
superioresDelCazadorL n1 (Explorador n2 ts l1 l2)    = superioresDelCazadorL n1 l1 ++ superioresDelCazadorL n1 l2
superioresDelCazadorL n1 (Cazador    n2 ps l1 l2 l3) = 
	if n1 == n2
	    then subordinadosQueSuperan (length ps) (l1:l2:l3:[])
        else superioresDelCazadorL n1 l1 ++ superioresDelCazadorL n1 l2 ++ superioresDelCazadorL n1 l3

subordinadosQueSuperan :: Int -> [Lobo] -> [Nombre]
subordinadosQueSuperan n []     = []
subordinadosQueSuperan n (l:ls) = losQueSuperan n l ++ subordinadosQueSuperan n ls

losQueSuperan :: Int -> Lobo -> [Nombre]
losQueSuperan x (Cria       n)             = []
losQueSuperan x (Explorador n ts l1 l2)    = losQueSuperan x l1 ++ losQueSuperan x l2
losQueSuperan x (Cazador    n ps l1 l2 l3) = 
	singularSi n (length ps > x) ++ (losQueSuperan x l1 ++ losQueSuperan x l2 ++ losQueSuperan x l3)
